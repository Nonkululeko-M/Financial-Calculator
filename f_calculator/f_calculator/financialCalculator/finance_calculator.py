...
if user_choice in user_investment:
    try:
        ...
        period_of_investment = int(input("How long are you planning to invest? (Months): "))  # Change to months
        interest = input("Do you want 'simple' or 'compound' interest: ").lower()

        if interest == "simple":
            future_amount = round(user_deposit_amount * (1 + interest_rate * period_of_investment / 1200), 2)
            print("The amount you will have is: R%.2f" % future_amount)
        elif interest == "compound":
            future_amount = round(user_deposit_amount * math.pow(1 + interest_rate / 1200, period_of_investment), 2)
    ...
    try:
        ...
        annual_interest_rate_input = input("What is the annual interest rate? (e.g., 7.7 or 7, not 7%): ")

        if "." in annual_interest_rate_input:
            annual_interest_rate = float(annual_interest_rate_input)
        else:
            annual_interest_rate = int(annual_interest_rate_input)
        ...
        print("You will need to pay monthly: R%.2f" % monthly_payment)
